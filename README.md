...
[![Elastic Stack 
version](https://img.shields.io/badge/Elastic%20Stack-8.7.1-00bfb3?style=flat&logo=elastic-stack)](https://www.elastic.co/blog/category/releases)
[![Build 
Status](https://github.com/deviantony/docker-elk/workflows/CI/badge.svg?branch=tls)](https://github.com/deviantony/docker-elk/actions?query=workflow%3ACI+branch%3Atls)
...
docker-compose up setup
```
After that we need to modify the .ENV file so that it reflects the real https URLs that we will be using to our production envornment. These 
will be the URLs that will be used to generate the LetsEncrypt Certificates
Open the .ENV file in any editor of your choice
```
nano .env
```
Edit the following lines to your URLs
```
ELASTIC_URL=
KIBANA_URL=
```
Example:
```
ELASTIC_URL=https://elastic.yourdomain.com
KIBANA_URL=https://kibana.yourdomain.com
```
**NOTE**: In order for this to work, we should create a DNS A record for your domain to point to the Public IP Address for our server.

**For Example**: If we are using the values in the config example above and your public IP is `100.100.100.100`
Create these two A records:
```
*Record*            *Value*         *IP Address*
A                   kibana          100.100.100.100
A                   elastic         100.100.100.100
```
...
logstash:
  ...
  environment:
    LS_JAVA_OPTS: -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false 
-Dcom.sun.management.jmxremote.port=18080 -Dcom.sun.management.jmxremote.rmi.port=18080 -Djava.rmi.server.hostname=DOCKER_HOST_IP 
