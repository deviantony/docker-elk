...
3. A [Filebeat](filebeat/filebeat.yml) ships the logs to [Logstash](logstash/pipeline) which parses and sends to [Elasticsearch](elasticsearch/config/elasticsearch.yml).
4. A jenkins job runs the following script: https://github.com/hydroshare/docker-elk/blob/main/jinja-report/build-report-jinja.py, which pulls data from elasticsearch and generates plots and puts them into [Caddy](caddy/Caddyfile) (used basically like an FTP)
5. In addition to the static metrics reports, one can use [Kibana](kibana/config/kibana.yml)
