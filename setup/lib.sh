...
function wait_for_elasticsearch {
	local elasticsearch_host="${ELASTICSEARCH_HOST:-elasticsearch01}"

	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}' 'https://elasticsearch01:9200/'
		'--resolve' "elasticsearch01:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		)
...
function wait_for_builtin_users {
	...
	local -a args=( '-s' '-D-' '-m15' 'https://elasticsearch01:9200/_security/user?pretty'
		'--resolve' "elasticsearch01:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		)
...
function check_user_exists {
	...
	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"https://elasticsearch01:9200/_security/user/${username}"
		'--resolve' "elasticsearch01:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		)
...
function set_user_password {
	...
	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"https://elasticsearch01:9200/_security/user/${username}/_password"
		'--resolve' "elasticsearch01:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		'-X' 'POST'
...
function ensure_role {
	...
	local -a args=( '-s' '-D-' '-m15' '-w' '%{http_code}'
		"https://elasticsearch01:9200/_security/role/${name}"
		'--resolve' "elasticsearch01:9200:${elasticsearch_host}" '--cacert' "$es_ca_cert"
		'-X' 'POST'
		'-H' 'Content-Type: application/json'
		'-d' "$body"
